...
from drp.core import EvalIterHook, build_optimizers
from drp.datasets.builder import build_dataloader, build_dataset
from drp.utils import get_root_logger
from torch_geometric.nn.data_parallel import DataParallel
from ..models.ME_DataParallel import MEDataParallel
...
def _non_dist_train(model,
    ...
    if validate and cfg.get('evaluation', None) is not None:
        ...
        runner.register_hook(
            EvalIterHook(data_loader, **cfg.evaluation), priority=80)
...
def _parallel_train(model,
    ...
    data_loaders = [build_dataloader(ds, parallel=True, **train_loader_cfg) for ds in dataset]

    # put model on gpus
    model = MEDataParallel(model, device_ids=cfg.device_ids,
                           follow_batch=train_loader_cfg['follow_batch'],
                           exclude_keys=train_loader_cfg['exclude_keys']).cuda()
    ...
    if validate and cfg.get('evaluation', None) is not None:
        ...
        data_loader = build_dataloader(dataset, parallel=True, **val_loader_cfg)
        # save_path = osp.join(cfg.work_dir, 'val_visuals')
        runner.register_hook(
            EvalIterHook(data_loader, **cfg.evaluation), priority=80)
