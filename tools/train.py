...
from drp.apis import set_random_seed, train_model
from drp.datasets import build_dataset
from drp.models import build_model
from drp.utils import collect_env, get_root_logger
...
def main():
    ...
    if args.launcher == 'none':
        ...
        if args.gpus > 1:
            parallel = True
            cfg.device_ids = [i for i in range(0, args.gpus)]
    else:
        distributed = True
    ...
    logger.info('parallel training: {}'.format(parallel))
    logger.info('Config:\n{}'.format(cfg.text))
